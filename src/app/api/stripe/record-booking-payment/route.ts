...
import { recordBookingPaymentSchema } from "@/lib/schemas/stripe";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsed = await validateBody(request, recordBookingPaymentSchema);
    ...
    // 5. Verify intent status with Stripe
    let paymentMethodId: string | null = body.stripe_payment_method_id || null;

    if (body.intent_type === "payment") {
      const pi = await stripe.paymentIntents.retrieve(body.intent_id, {}, {
      ...
      const si = await stripe.setupIntents.retrieve(body.intent_id, {}, {
        stripeAccount: settings.stripe_account_id,
      } as Stripe.RequestOptions);
    ...
    // 6. Determine status and charge_type
